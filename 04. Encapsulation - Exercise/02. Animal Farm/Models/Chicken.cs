namespace _02._Animal_Farm.Models
{
    using Contracts;
    using Utilities.Messages;
    public class Chicken : IChicken
    {
        public const int MinAge = default;
        public const int MaxAge = 15;
        private string _name;
        private int _age;
        public Chicken(string name, int age)
        {
            Name = name;
            Age = age;
        }
        public string Name
        {
            get => _name;
            private set
            {
                if (string.IsNullOrWhiteSpace(value))
                    throw new NameArgumentsCannotBeNullOrWhiteSpace();
                _name = value;
            }
        }
        public int Age
        {
            get => _age;
            private set
            {
                if (value < MinAge || value > MaxAge)
                    throw new AgeArgumentValueShouldBeBetween0To15();
                _age = value;
            }
        }
        public double ProductPerDay => CalculateProductPerDay();

        public double CalculateProductPerDay()
        {
            switch (this.Age)
            {
                case 0:
                case 1:
                case 2:
                case 3:
                    return 1.5;
                case 4:
                case 5:
                case 6:
                case 7:
                    return 2;
                case 8:
                case 9:
                case 10:
                case 11:
                    return 1;
                default:
                    return 0.75;
            }
        }
        public override string ToString()
            => $"Chicken {Name} (age {Age}) can produce {ProductPerDay} eggs per day.";
    }
}
